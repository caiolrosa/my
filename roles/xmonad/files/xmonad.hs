...
import XMonad.Hooks.DynamicLog (dynamicLogWithPP, xmobarPP, wrap, xmobarColor, PP(..))
...
--
myWorkspaces    = ["web","terminal","music"]
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm              , xK_v), spawn "amixer -q set Master 2+")
    , ((modm .|. shiftMask, xK_v), spawn "amixer -q set Master 2-")
    , ((modm .|. shiftMask, xK_m), spawn "amixer -D pulse set Master toggle")
...
main = do
	...
	xmonad $ ewmh def
		...
		, logHook            = dynamicLogWithPP $ xmobarPP
			{ ppOutput          = \x -> hPutStrLn xmproc $ wrap "  " "  " x
